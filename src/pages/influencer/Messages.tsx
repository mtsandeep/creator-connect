  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'messages'), {
        ...
        senderId: user.uid,
        receiverId: proposal.promoterId,
    ...
    <div className="h-full flex">
      ...
      <div className={`w-full lg:w-96 border-r border-white/10 flex flex-col ${selectedProposalId ? 'hidden lg:flex' : 'flex'}`}>
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-gray-400 text-sm mt-1">Chat with brands about collaborations</p>
        </div>
          ...
          <div className="flex-1 flex items-center justify-center p-8 text-center">
            <div>
              ...
              <p className="text-gray-400 text-sm">
                Start chatting with brands when you accept proposals
              </p>
            </div>
          </div>
        ) : (
            ...
            {conversations.map((conversation) => (
              <button
                ...
                className={`w-full p-4 text-left hover:bg-white/5 transition-colors border-b border-white/10 ${
                  selectedProposalId === conversation.id ? 'bg-[#00D9FF]/10 border-l-2 border-l-[#00D9FF]' : ''
                }`}
              >
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-[#B8FF00] to-[#B8FF00]/50 rounded-full flex items-center justify-center flex-shrink-0">
                    <span className="text-sm font-bold text-gray-900">
                      {(conversation.promoterName || 'Brand')[0].toUpperCase()}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-white font-medium truncate">
                        {conversation.promoterName || 'Brand'}
                      </h3>
                      {conversation.unreadCount > 0 && (
                        <span className="bg-[#00D9FF] text-gray-900 text-xs font-bold px-2 py-0.5 rounded-full">
          ...
          <>
            ...
            <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-[#B8FF00] to-[#B8FF00]/50 rounded-full flex items-center justify-center">
                  <span className="text-sm font-bold text-gray-900">
                    {(selectedConversation.promoterName || 'Brand')[0].toUpperCase()}
                  </span>
                </div>
                <div>
                  <h3 className="text-white font-medium">{selectedConversation.promoterName || 'Brand'}</h3>
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <textarea
                    ...
                    rows={1}
                    className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#00D9FF] resize-none"
                ...
                <button
                  ...
                  className="bg-[#00D9FF] hover:bg-[#00D9FF]/80 disabled:bg-white/10 disabled:text-gray-500 text-gray-900 font-semibold p-3 rounded-xl transition-colors flex-shrink-0"
                >
