...
export default function PromoterBusinessProfile() {
  const { user } = useAuthStore();
  const navigate = useNavigate();
  const { roleData, isComplete, loading, error, save } = useBusinessProfile('promoter');
    ...
    <div className="p-8 max-w-3xl mx-auto">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6 space-y-4">
          <div>
            <label className="block text-sm text-gray-400 mb-2">Legal Entity Name</label>
            <input
              value={legalName}
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-2">Business PAN</label>
            <input
              value={pan}
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#B8FF00] resize-none"
        ...
        <div className="flex gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex-1 bg-[#B8FF00] hover:bg-[#B8FF00]/80 text-gray-900 font-semibold py-3 rounded-xl transition-colors disabled:opacity-50"
          >
            {loading ? 'Saving...' : 'Save'}
          </button>
        </div>
